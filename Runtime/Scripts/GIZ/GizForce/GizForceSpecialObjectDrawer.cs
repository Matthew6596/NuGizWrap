...
{
    using Helper;
    [CustomPropertyDrawer(typeof(GizForce.SpecialObject))]
    public class GizForceSpecialObjectDrawer : PropertyDrawer
    {
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            ...
            int version = GizmoConfig.Instance.gizForce.version;

            float height = EditorGUIUtility.singleLineHeight * 4;
            if (specObjVers >= 2) height += EditorGUIUtility.singleLineHeight;
            if (version >= 9) height += EditorGUIUtility.singleLineHeight;
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (version >= 9) property.Prop("unknown3", specObjVers >= 2 ? unk3Rect : unk2Rect);

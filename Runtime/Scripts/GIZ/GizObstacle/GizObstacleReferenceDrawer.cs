...
{
    [CustomPropertyDrawer(typeof(GizObstacleReference))]
    public class GizObstacleReferenceDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            var nameProp = property.FindPropertyRelative("gizObstacle");
            ...
            refProp.boolValue = EditorGUI.Toggle(toggleRect, new GUIContent("", "Check this to reference a GizObstacle in the scene. Uncheck to enter GizObstacle name manually."), refProp.boolValue);
            ...
            if (refProp.boolValue)
            {
                ...
                if (wasNameInput)
                {
                    var blwup = GameObject.FindObjectsByType<GizObstacle>(FindObjectsInactive.Exclude, FindObjectsSortMode.None).Where(b => b.name == nameProp.stringValue).FirstOrDefault();
